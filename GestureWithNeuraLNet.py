...
class HandGestureController:
    ...
    def preprocess_landmarks(self, hand_landmarks):
        """
        Preprocess hand landmarks for prediction.
        """
        landmarks = np.array([[lm.x, lm.y, lm.z] for lm in hand_landmarks.landmark])
        landmarks = landmarks.flatten()

        # Pad or reshape the landmarks to match the model's input size
    ...
    def predict_gesture(self, hand_landmarks):
        """
        Predict the gesture using the trained neural network model.
        """
        ...
        print(f"Predicted gesture: {gesture} with confidence scores: {prediction}")  # Debugging output
        return gesture
    ...
    def perform_action(self, right_fingers, left_fingers):
      ...
      if right_fingers == 1:
        ...
        elif right_fingers == 1 and left_fingers is not None and left_fingers == 2:
            print("Action: Fast Forward")
            pyautogui.hotkey('shift', 'right')
        elif right_fingers == 1 and left_fingers is not None and left_fingers == 3:
            print("Action: Rewind")
